...
export default function AdminOrders() {
  ...
  const getStatusStyle = (status: string) => {
    const styles: Record<string, { bg: string; text: string; border: string; icon: string }> = {
      pending: { bg: "bg-amber-50", text: "text-amber-800", border: "border-amber-300", icon: "⏳" },
      processing: { bg: "bg-sky-50", text: "text-sky-800", border: "border-sky-300", icon: "🔨" },
      shipped: { bg: "bg-violet-50", text: "text-violet-800", border: "border-violet-300", icon: "📦" },
      delivered: { bg: "bg-emerald-50", text: "text-emerald-800", border: "border-emerald-300", icon: "✨" },
      cancelled: { bg: "bg-rose-50", text: "text-rose-800", border: "border-rose-300", icon: "✕" },
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f5ebe0] via-[#faf7f2] to-[#e8d5c4] relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
        ...
        <svg className="absolute top-20 left-10 opacity-10" width="200" height="200" viewBox="0 0 200 200">
          <path d="M10,100 Q50,50 100,100 T190,100" stroke="#8b6f47" strokeWidth="2" fill="none" strokeDasharray="5,5" />
        </svg>
        <svg className="absolute bottom-40 right-20 opacity-10" width="150" height="150" viewBox="0 0 150 150">
          <path d="M0,75 Q37.5,25 75,75 T150,75" stroke="#d4a574" strokeWidth="2" fill="none" strokeDasharray="4,4" />
        </svg>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-12 md:px-8 md:py-16">
        ...
        <header className="text-center mb-16 relative">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-4 text-[#d4a574]/60 text-4xl">✦</div>
          ...
          <div className="inline-block relative">
            ...
            <div className="relative bg-[#faf7f2]/80 backdrop-blur-sm px-12 py-8 border-2 border-[#8b6f47]/40">
              ...
              <Title level={1} className="!font-serif !text-[#5d4037] !mb-2 !text-4xl md:!text-5xl tracking-wide !font-normal" style={{ fontVariant: 'small-caps' }}>
                Order Ledger
              </Title>
          ...
          <div className="mt-8 flex items-center justify-center gap-3 text-[#8b6f47]/50">
            ...
            <span className="text-sm font-serif italic">{orders?.length || 0} tales recorded</span>
            <span className="text-lg">🧶</span>
          </div>
        </header>
            ...
            {orders?.map((order: any, index: number) => {
              ...
              return (
                <article key={order.id} className="group relative">
                  ...
                  <div className="absolute -top-3 left-8 right-8 h-6 bg-gradient-to-r from-[#d4a574]/60 via-[#e8d5c4]/80 to-[#d4a574]/60 transform -rotate-1 z-20 shadow-sm" 
                  ...
                  <div className="relative bg-gradient-to-br from-[#fffef9] via-[#fdfcf7] to-[#f8f4ed] rounded shadow-lg border border-[#e0d5c4] overflow-hidden transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-xl">
                    ...
                    <div className="absolute top-6 right-6 z-10">
                      <div className="relative">
                        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#8b6f47] via-[#a67c52] to-[#5d4037] shadow-lg flex items-center justify-center transform rotate-12 group-hover:rotate-6 transition-transform duration-300">
                          <div className="text-center">
                            <div className="text-[#f5ebe0] text-[10px] uppercase tracking-wider font-bold">No.</div>
                            <div className="text-[#f5ebe0] text-lg font-serif font-bold leading-none">{order.id}</div>
                          </div>
                        </div>
                    ...
                    <div className="p-8 pt-10">
                      <div className="mb-6">
                        <Text className="text-[#9ca3af] text-xs uppercase tracking-widest block mb-1">Inscribed on</Text>
                      ...
                      <div className="mb-6 pl-4 border-l-2 border-[#e8d5c4]">
                        <Text className="text-[#9ca3af] text-[10px] uppercase tracking-[0.2em] block mb-1">For the Esteemed</Text>
                        <Text className="text-[#4b5563] font-serif text-xl block truncate" title={order.user?.email}>
                          {order.user?.firstName || order.user?.email?.split('@')[0] || 'Valued Patron'}
                        </Text>
                      ...
                      <div className="bg-[#faf7f2]/50 rounded-sm p-4 mb-6 border border-dashed border-[#e0d5c4]">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">📦</span>
                          <div>
                            <Text className="text-[#9ca3af] text-[10px] uppercase tracking-widest block">Treasures Contained</Text>
                            <Text className="text-[#5d4037] font-serif text-lg">
                              {order.items?.length || 0} {order.items?.length === 1 ? 'Handcrafted Piece' : 'Handcrafted Pieces'}
                            </Text>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="relative bg-[#5d4037] text-[#f5ebe0] p-4 -mx-8 mb-6">
                        ...
                        <div className="flex justify-between items-center px-4">
                          <Text className="text-[#d4a574] font-serif italic">Sum Total</Text>
                          <div className="flex items-baseline">
                            <span className="text-[#d4a574] text-sm mr-1 font-serif">$</span>
                      ...
                      <div className="relative">
                        ...
                        <div className={`${statusStyle.bg} ${statusStyle.border} border-2 rounded-sm p-1`}>
                          <Select
                            value={order.status}
                            onChange={(value) => updateStatusMutation.mutate({ orderId: order.id, status: value })}
                            className="w-full"
                            variant="borderless"
                            suffixIcon={<span className="text-lg">{statusStyle.icon}</span>}
                          >
